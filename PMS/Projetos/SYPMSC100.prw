...
±±³Funcao    ³ SYPMSC100 ³ Autor ³ Cris Barroso         ³ Data ³27/04/2012³±±
...
Else
	...
	IF DbSeek(xFilial("AE8")+__cUserID)
		...
		ElseIF AE8->AE8_EQUIPE $ "357" //adm/comercial/pmo
...
ncountHr := ncountPr := ncountTf := ncountOk := ncount40 := ncount70 := nCountAv:= nCountAb:= ncountBl:= nCountSy := nCountEn:= nCountAd:= 0 
...
MsgRun("Aguarde, Filtrando Projetos...",, {|| CursorWait(), U_SYPMSDados(,,,;
@aColsPrj,@aHeadPRJ,;
lDiretoria,lPMO,;
...
DEFINE MSDIALOG oTelaPRJ FROM aSize[7],0 To aSize[6],aSize[5] TITLE "Painel de Projetos" OF oMainWnd PIXEL
...
oFolder:=TFolder():New(0,0,{"Indicadores",;
...
"Estourados: "			+Alltrim(Str(ncountHr)),;
"Bloqueados: " 			+Alltrim(Str(ncountBl)),;
"Atendimento Avulso: "	+Alltrim(Str(ncountAv)),;
"Banco de Horas: "		+Alltrim(Str(ncountAb)),;
"Symm: "				+Alltrim(Str(ncountSy)),;
"Encerrados: "			+Alltrim(Str(ncountEn))};
,,oTelaPRJ,,,,.T.,.F.,0,0)

oFolder:Align := CONTROL_ALIGN_ALLCLIENT

oPanel1				:= TPanel():New(0, 0, '', oFolder:aDialogs[2], NIL, .T., .F., NIL,, 0,0, .T., .F. )
oPanel1:Align		:= CONTROL_ALIGN_ALLCLIENT

oPanel2				:= TPanel():New(0, 0, '', oFolder:aDialogs[2], NIL, .F., .F., NIL,, 0,30, .T., .F. )
...
//³ Projetos Ok.                                         ³
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
oPrjOk							:= MsNewGetDados():New(0,0,0,0,nStyle,"Allwaystrue","Allwaystrue","",aAlter,,,,,,oFolder:aDialogs[3],aHeadOkPRJ,aColsOkPRJ)
...
oPanelOk			:= TPanel():New(0, 0, '', oFolder:aDialogs[3], NIL, .T., .F., NIL,, 0,100, .F., .T. )
...
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
oPrjPr							:= MsNewGetDados():New(0,0,0,0,nStyle,"Allwaystrue","Allwaystrue","",aAlter,,,,,,oFolder:aDialogs[4],aHeadPrPRJ,aColsPrPRJ)
...
oPanelPr			:= TPanel():New(0, 0, '', oFolder:aDialogs[4], NIL, .T., .F., NIL,, 0,100, .F., .T. )
...
//³ Projetos Atingiu 40%.                                ³
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
oPrj40							:= MsNewGetDados():New(0,0,0,0,nStyle,"Allwaystrue","Allwaystrue","",aAlter,,,,,,oFolder:aDialogs[5],aHead40PRJ,aCols40PRJ)
...
oPanel40			:= TPanel():New(0, 0, '', oFolder:aDialogs[5], NIL, .T., .F., NIL,, 0,100, .F., .T. )
...
//³ Projetos Atingiu >= 70%.                             ³
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
oPrj70							:= MsNewGetDados():New(0,0,0,0,nStyle,"Allwaystrue","Allwaystrue","",aAlter,,,,,,oFolder:aDialogs[6],aHead70PRJ,aCols70PRJ)
...
oPanel70			:= TPanel():New(0, 0, '', oFolder:aDialogs[6], NIL, .T., .F., NIL,, 0,100, .F., .T. )
...
//³ Projetos Estourados.                                 ³
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
oPrjHr							:= MsNewGetDados():New(0,0,0,0,nStyle,"Allwaystrue","Allwaystrue","",aAlter,,,,,,oFolder:aDialogs[7],aHeadHrPRJ,aColsHrPRJ)
...
oPanelHr			:= TPanel():New(0, 0, '', oFolder:aDialogs[7], NIL, .T., .F., NIL,, 0,100, .F., .T. )
...
//³ Projetos Bloqueados.                                 ³
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
oPrjBl							:= MsNewGetDados():New(0,0,0,0,nStyle,"Allwaystrue","Allwaystrue","",aAlter,,,,,,oFolder:aDialogs[8],aHeadBlPRJ,aColsBlPRJ)
...
oPanelBl			:= TPanel():New(0, 0, '', oFolder:aDialogs[8], NIL, .T., .F., NIL,, 0,100, .F., .T. )
...
//³ Projetos Avulsos.                                 ³
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
oPrjAvulso							:= MsNewGetDados():New(0,0,0,0,nStyle,"Allwaystrue","Allwaystrue","",aAlter,,,,,,oFolder:aDialogs[9],aHeadAvulso,aColsAvulso)
...
oPanelAv			:= TPanel():New(0, 0, '', oFolder:aDialogs[9], NIL, .T., .F., NIL,, 0,100, .F., .T. )
...
//³ Projetos Abertos.                                 ³
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
oPrjAberto							:= MsNewGetDados():New(0,0,0,0,nStyle,"Allwaystrue","Allwaystrue","",aAlter,,,,,,oFolder:aDialogs[10],aHeadAberto,aColsAberto)
...
oPanelAb			:= TPanel():New(0, 0, '', oFolder:aDialogs[10], NIL, .T., .F., NIL,, 0,100, .F., .T. )
...
//³ Projetos Abertos.                                 ³
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
oPrjSymm							:= MsNewGetDados():New(0,0,0,0,nStyle,"Allwaystrue","Allwaystrue","",aAlter,,,,,,oFolder:aDialogs[11],aHeadSymm,aColsSymm)
...
oPanelSy			:= TPanel():New(0, 0, '', oFolder:aDialogs[11], NIL, .T., .F., NIL,, 0,100, .F., .T. )
...
//³ Projetos Encerrados.                                 ³
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
oPrjEn							:= MsNewGetDados():New(0,0,0,0,nStyle,"Allwaystrue","Allwaystrue","",aAlter,,,,,,oFolder:aDialogs[12],aHeadEn,aColsEn)
...
oPanelEn			:= TPanel():New(0, 0, '', oFolder:aDialogs[12], NIL, .T., .F., NIL,, 0,100, .F., .T. )
...
ACTIVATE MSDIALOG oTelaPRJ ON INIT MyEnchoBar(oTelaPRJ,aButtons)
...
User Function SYPMSDados(cCliente,cLojaCli,cRecurso,aColsPrj,aHeadPRJ,lDiretoria,lPMO,;
aColsOkPRJ,aHeadOkPRJ,;
aColsPrPRJ,aHeadPrPRJ,;
aCols40PRJ,aHead40PRJ,;
aCols70PRJ,aHead70PRJ,;
...
cQuery3+= " LEFT OUTER JOIN #PROPOSTA ON AF8_PROPOS=Z02_PROPOS AND AF8_ADITIV =Z02_ADITIV"
...
cQuery14:= ""
//cQuery14:= " UPDATE PROJETO SET HRPREV=0,PERC=0,SALDO=0 WHERE SERVICO IN ('000001','000009') "
cQuery14+= " UPDATE PROJETO SET FLAG='Av' WHERE SERVICO = '000001' AND ENCERRA<>'1'  "     
cQuery14+= " UPDATE PROJETO SET FLAG='Ab' WHERE SERVICO IN ('000004','000005') AND ENCERRA<>'1'  "     
cQuery14+= " UPDATE PROJETO SET FLAG='Hr' WHERE REALIZA+TRANSLA>(HRPREV+HRAPORTE)  AND SERVICO NOT IN ('000001','000004','000005','000009')"
//cQuery14+= " UPDATE PROJETO SET FLAG='Pr' WHERE DATAFIN<'"+dtos(dDataBase)+"' AND FLAG=' ' AND DATAFIN<>' ' AND ENCERRA<>'1' AND SERVICO NOT IN ('000001','000004','000005','000009')"
cQuery14+= " UPDATE PROJETO SET FLAG='70' WHERE PERC>=70 AND FLAG=' ' AND ENCERRA<>'1' AND SERVICO NOT IN ('000001','000004','000005','000009')"
cQuery14+= " UPDATE PROJETO SET FLAG='40' WHERE PERC>=40 AND FLAG=' ' AND ENCERRA<>'1'  AND SERVICO NOT IN ('000001','000004','000005','000009')"
cQuery14+= " UPDATE PROJETO SET FLAG='Bl' WHERE BLQPRJ IN ('S')"
cQuery14+= " UPDATE PROJETO SET FLAG='Ok' WHERE FLAG=' '   AND SERVICO NOT IN ('000001','000004','000005','000009')"     
cQuery14+= " UPDATE PROJETO SET FLAG='Sy' WHERE FLAG=' '   AND SERVICO IN ('000009')"     
cQuery14+= " UPDATE PROJETO SET FLAG='En' WHERE ENCERRA = '1'  "     
cQuery14+= " UPDATE PROJETO SET FLAG='Ad' WHERE BLQPRJ IN ('A')"
TcSqlExec(cquery14) 
...
EndIF
Aadd(aRotina,{"Contatos","U_SyManCt(SA1->A1_COD+SA1->A1_LOJA)" , 0 , 6,82,NIL})
...
User Function SyManCt(cEntidade,cAliasTab)     
...
DbSetOrder(1)
DbSeek("AC8")
While !Eof() .And. (SX3->X3_ARQUIVO == "AC8")
	
	IF X3USO(SX3->X3_USADO) .And. cNivel >= SX3->X3_NIVEL
		IF ( AllTrim(X3_CAMPO) $ "AC8_CODCON" )
...
While !Eof() .And. (SX3->X3_ARQUIVO == "SU5")
	...
	IF X3USO(SX3->X3_USADO) .And. cNivel >= SX3->X3_NIVEL
		...
		IF ( AllTrim(X3_CAMPO) $ "U5_CONTAT/U5_CELULAR/U5_FCOM1/U5_EMAIL/U5_APRVOS" )
			AADD(aHeadCon,{ 	TRIM(x3titulo()), SX3->X3_CAMPO, SX3->X3_PICTURE, SX3->X3_TAMANHO,SX3->X3_DECIMAL, SX3->X3_VALID,;
								SX3->X3_USADO, SX3->X3_TIPO, SX3->X3_F3,SX3->X3_CONTEXT, SX3->X3_CBOX, SX3->X3_RELACAO, SX3->X3_WHEN})
...
cQuery := " SELECT U5_CODCONT AS AC8_CODCON, U5_CONTAT, U5_CELULAR, U5_FCOM1, U5_EMAIL, U5_APRVOS "
...
Else
	While !Eof()
		nStatus:= Val(&("CON->"+Alltrim(aHeadCon[6][2])))
		nStatus:= IIf(nStatus == 0,3,nStatus)

		AADD(aColsCon,{&("CON->"+Alltrim(aHeadCon[1][2])),;
		&("CON->"+Alltrim(aHeadCon[2][2])),&("CON->"+Alltrim(aHeadCon[3][2])),;
		&("CON->"+Alltrim(aHeadCon[4][2])),&("CON->"+Alltrim(aHeadCon[5][2])),;
		&("CON->"+Alltrim(aHeadCon[6][2])),.F.}  )
...
For nX:=1 To Len(oGetCon:aCols)
	If !oGetCon:aCols[nX,Len(oGetCon:aHeader)+1]
		...
		IF !DbSeek(xFilial("AC8")+oGetCon:aCols[nX,1]+"SA1"+xFilial("SA1")+cCliente)
			...
			Replace AC8_FILENT  With xFilial("SA1")
			Replace AC8_CODENT	With cCliente
			Replace AC8_CODCON	With oGetCon:aCols[nX,1]
			MsUnlock()
		EndIF
	EndIf
Next nX
...
User Function SyDocument( cAlias, cbusca, nOpc, nOper, oGetOp)
...
IF oGetOp != NIL
	cbusca 	:= oGetOp:aCols[oGetOp:nAt,aScan(oGetOp:aHeader,{|x| AllTrim(x[2]) == "Z02_PROPOS"})]
	nOk		:= aScan(oGetOp:aHeader,{|x| AllTrim(x[2]) == "Z02_OK2"})
EndIF
...
ElseIf cEntidade = "Z02"
	cSyDir :="PROPOST"+cbusca
ElseIf cEntidade = "AF8"
	cSyDir :="PROJETO"+cbusca
Endif

AAdd( aEntidade, { "SA1", { "A1_COD", "A1_LOJA" }		, { || SA1->A1_NOME } } )
AAdd( aEntidade, { "Z02", { "Z02_PROPOS", "Z02_ADITIV" }, { || Z02->Z02_RAZAO } } )
					...
					IF lGravou
						...
						IF oGetOp != NIL
							...
							For nX := 1 To Len(aCols)
								...
								oGetOp:aCols[oGetOp:nAt,nOK] := LoadBitmap(GetResources(),'CLOCK01')
								
								IF ( !Empty(aCols[nX,1]) .And. !GDDeleted(nX) )
								
									oGetOp:aCols[oGetOp:nAt,nOK] := LoadBitmap(GetResources(),'BR_VERDE')
...
User Function SyFLDelete(cAlias,nReg,nOpcx)
...
If Empty( GDFieldGet( "AC9_OBJETO" ) ) .Or. SyDocLOK() 
	...
	aColsWiz[1,nUsado+1] := .F.
		...
		oGetD := MsNewGetDados():New(55, 50, 115, 250, GD_INSERT+GD_UPDATE+GD_DELETE,,,,,,100,,,,oWizard:GetPanel(2),aHeaderWIZ,aColsWiz)
...
oScroll:= TScrollBox():New(oFolder:aDialogs[1],0,0,0,0,.T.,.T.)
...
MemoWrite('C:\Propostas\MetaOsQuery.txt',cQuery)
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
*/
User Function ObsView(oMemo,cMemo,oPrj)
Local aArea    := GetArea()
Local cObsPrj  := ""
Local cProjeto := ""
