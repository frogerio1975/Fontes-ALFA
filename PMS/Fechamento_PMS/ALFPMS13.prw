...
//-------------------------------------------------------------------
/*/{Protheus.doc} ALFPMS13
 
Descricao: RELATORIO Colaborador PMS 
...
/*/
//-------------------------------------------------------------------
User Function ALFPMS13( cGrupo )


Local aArea 	:= GetArea()
Local cDir 		:= AllTrim(GetTempPath())
Local cArquivo 	:= "relatorio"
Local cTitulo 	:= "Relatório de Colaborador PMS"
Local cNome 	:= "RelColaborador PMS-"+DtoS(Date())+"-"+STRTRAN(TIME(),":","")
Local cDesc 	:= "Esta rotina tem como objetivo criar um arquivo no formato XML Excel contendo relatório de Colaborador PMS."
Local cExt 		:= "XLS"
...
IF ParamBox(aParamBox,"Informe o recurso para Filtro",@aRetParam)
    cRecurso  := aRetParam[1]
    cCompIni  := aRetParam[2]
    cCompFim  := aRetParam[3]
...
oXml:AddRow(, {"Competencia"     , cRecurso }, aStl)
oXml:AddRow(, {"Competencia"     , cCompIni }, aStl)
oXml:AddRow(, {"Revisao"         , cCompFim }, aStl)
...
oXml:setFolder(nFolder)
nFolder++
oXml:setFolderName("Colaborador PMS")
oXml:showGridLine(.F.)
oXml:SetZoom(100)
...
aAdd( aCabTit, "Relatório de Colaborador PMS " ) // Data emissão
...
While (cTMP1)->(!EOF())
    ...
    aAdd( aRowDad, (cTMP1)->ZH_COMPETE+'-'+(cTMP1)->ZH_REVISAO+' - '+(cTMP1)->ZH_DESCRI ) 
