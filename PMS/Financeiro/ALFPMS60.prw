...
//-------------------------------------------------------------------
/*/{Protheus.doc} ALFPMS60
Agenda pagamento de comissões.
...
cQuery := " SELECT "+ CRLF
cQuery += "     Z39.Z39_FILIAL "+ CRLF
cQuery += "     ,Z39.Z39_TIPO "+ CRLF
cQuery += "     ,Z39.Z39_DTGERA "+ CRLF
cQuery += "     ,Z39.Z39_PROPOS "+ CRLF
cQuery += "     ,Z39.Z39_ADITIV "+ CRLF
cQuery += "     ,Z39.Z39_CODCLI "+ CRLF
cQuery += "     ,Z39.Z39_LOJCLI "+ CRLF
cQuery += "     ,Z39.Z39_MOD "+ CRLF
cQuery += "     ,Z39.Z39_VEND "+ CRLF
cQuery += "     ,Z39.Z39_E1PREF "+ CRLF
cQuery += "     ,Z39.Z39_E1NUM "+ CRLF
cQuery += "     ,Z39.Z39_E1PARC "+ CRLF
cQuery += "     ,Z39.Z39_E1TIPO "+ CRLF
cQuery += "     ,Z39.Z39_E1BAIX "+ CRLF
cQuery += "     ,Z39.Z39_VLRBRU "+ CRLF
cQuery += "     ,Z39.Z39_IMPOST "+ CRLF
cQuery += "     ,Z39.Z39_VLRLIQ "+ CRLF
cQuery += "     ,Z39.Z39_COMISS "+ CRLF
cQuery += "     ,Z39.Z39_VLRCOM "+ CRLF
cQuery += "     ,Z39.Z39_STATUS "+ CRLF
cQuery += "     ,Z39.Z39_E1HIST "+ CRLF
...
cQuery += "     AND SA3.A3_COD = Z39.Z39_VEND "+ CRLF
cQuery += "     AND SA3.D_E_L_E_T_ = ' ' "+ CRLF

cQuery += " LEFT JOIN "+RetSqlName("SA1")+" SA1 (NOLOCK) "+ CRLF
cQuery += "     ON SA1.A1_FILIAL = '"+xFilial("SA1")+"' "+ CRLF
cQuery += "     AND SA1.A1_COD = Z39.Z39_CODCLI "+ CRLF
cQuery += "     AND SA1.A1_LOJA = Z39.Z39_LOJCLI "+ CRLF
cQuery += "     AND SA1.D_E_L_E_T_ = ' ' "+ CRLF

cQuery += " WHERE "+ CRLF
cQuery += " 	Z39.Z39_FILIAL = '"+xFilial("Z39")+"' "+ CRLF
cQuery += " 	AND Z39.Z39_E1BAIX BETWEEN '"+DToS(dBaixaDe)+"' AND '"+DToS(dBaixaAte)+"' "+ CRLF
cQuery += "     AND Z39.Z39_STATUS IN ('1') "+ CRLF
...
AADD( aSetField, { "Z39_DTGERA", "D", 08, 0 } )
AADD( aSetField, { "Z39_E1BAIX", "D", 08, 0 } )
AADD( aSetField, { "Z39_VLRBRU", "N", 12, 2 } )
AADD( aSetField, { "Z39_IMPOST", "N", 12, 4 } )
AADD( aSetField, { "Z39_VLRLIQ", "N", 12, 2 } )
AADD( aSetField, { "Z39_COMISS", "N", 12, 2 } )
AADD( aSetField, { "Z39_VLRCOM", "N", 12, 2 } )

cTMP1 := MPSysOpenQuery(cQuery,,aSetField)

While (cTMP1)->(!EOF())

    cChave := (cTMP1)->(Z39_VEND)
    ...
    aVendedor[nPosVnd][C1_VEND]   := (cTMP1)->Z39_VEND
    ...
    While (cTMP1)->(!EOF()) .And. cChave == (cTMP1)->(Z39_VEND)

        aTitAux := Array(C2_EMPTY)

        DO CASE
            CASE (cTMP1)->Z39_TIPO == "1"
                cTipoCom := "Proposta"
            CASE (cTMP1)->Z39_TIPO == "2"
                cTipoCom := "Apontamento"
            CASE (cTMP1)->Z39_TIPO == "3"
                cTipoCom := "Avulso"
            OTHERWISE
                cTipoCom := (cTMP1)->Z39_TIPO
        END CASE
        
        //1=Servicos;2=Produtos;3=Setup Cloud;4=Parcelas Mensais;5=Suporte Mensal
        DO CASE
            CASE (cTMP1)->Z39_MOD == "1"
                cModalidade := "Servicos"
            CASE (cTMP1)->Z39_MOD == "2"
                cModalidade := "Produtos"
            CASE (cTMP1)->Z39_MOD == "3"
                cModalidade := "Setup Cloud"
            CASE (cTMP1)->Z39_MOD == "4"
                cModalidade := "Parcelas Mensais"
            CASE (cTMP1)->Z39_MOD == "5"
                cModalidade := "Suporte Mensal"
            OTHERWISE
                cModalidade := (cTMP1)->Z39_MOD
        END CASE

        aTitAux[C2_MARK]   := "LBOK"
        aTitAux[C2_TIPO]   := cTipoCom
        aTitAux[C2_PROPOS] := (cTMP1)->Z39_PROPOS
        aTitAux[C2_ADITIV] := (cTMP1)->Z39_ADITIV
        aTitAux[C2_CODCLI] := (cTMP1)->Z39_CODCLI
        aTitAux[C2_LOJCLI] := (cTMP1)->Z39_LOJCLI
        aTitAux[C2_MOD]    := cModalidade
        aTitAux[C2_NOMCLI] := (cTMP1)->A1_NOME
        aTitAux[C2_E1PREF] := (cTMP1)->Z39_E1PREF
        aTitAux[C2_E1NUM ] := (cTMP1)->Z39_E1NUM
        aTitAux[C2_E1PARC] := (cTMP1)->Z39_E1PARC
        aTitAux[C2_E1TIPO] := (cTMP1)->Z39_E1TIPO
        aTitAux[C2_E1BAIX] := (cTMP1)->Z39_E1BAIX
        aTitAux[C2_VLRTIT] := (cTMP1)->Z39_VLRBRU
        aTitAux[C2_IMPOST] := (cTMP1)->Z39_IMPOST
        aTitAux[C2_VLRLIQ] := (cTMP1)->Z39_VLRLIQ
        aTitAux[C2_COMISS] := (cTMP1)->Z39_COMISS
        aTitAux[C2_VLRCOM] := (cTMP1)->Z39_VLRCOM
        aTitAux[C2_HISTOR] := (cTMP1)->Z39_E1HIST
        aTitAux[C2_RECZ39] := (cTMP1)->RECZ39
        aTitAux[C2_EMPTY]  := ""

        AADD( aComissao[nPosVnd], aTitAux )

        nVlrTotal += (cTMP1)->Z39_VLRCOM
...
If ParamBox(aBoxParam,"Informe os Parametros",@aRetParam,,,,,,,,.F.)
    ...
    If lRetorno
        For nItem := 1 To Len(aTitulo)
            If aTitulo[nItem][C2_MARK] == "LBOK"
                DbSelectArea("Z39")
                DbGoTo(aTitulo[nItem][C2_RECZ39])
                RecLock("Z39",.F.)
                    REPLACE Z39_E2PREF WITH cPrefixo
                    REPLACE Z39_E2NUM  WITH cNumTit
                    REPLACE Z39_E2PARC WITH cParcela
                    REPLACE Z39_E2TIPO WITH cTipo
                    REPLACE Z39_STATUS WITH "2"
                MsUnlock()
            EndIf
        Next nItem
