...
@version 1.0
	...
	15 - MATA061 - SA5 - Produtos x Fornecedor
	@see http://terminaldeinformacao.com
/*/
User Function zAltGen()
	Local aArea := GetArea()
	//Dimensões da janela
	Local nJanAltu := 180
	Local nJanLarg := 650
	...
	Local cAviso := ""
	Private aIteTip := {}
	Private aIteAcao := {"3=Inclusão","4=Alteração"}
	Private oSayArq, oGetArq, cGetArq := Space(99)
	Private oSayTip, oCmbTip, cCmbTip := ""
	Private oSayAcao, oCmbAcao, cCmbAcao := ""
	...
	*/
	...
	//cAviso += " 15 - MATA061 - SA5 - Produtos x Fornecedor"+STR_PULA
	cAviso += "--"+STR_PULA
	...
	DEFINE MSDIALOG oDlgPvt TITLE "Carga Dados - Genérico" FROM 000, 000  TO nJanAltu, nJanLarg COLORS 0, 16777215 PIXEL
		...
		@ 003, 003 	GROUP oGrpPar TO 060, (nJanLarg/2) 	PROMPT "Parâmetros: " 		OF oDlgPvt COLOR 0, 16777215 PIXEL
			...
			@ 028, 190 SAY        oSayAcao PROMPT "Ação:"          SIZE 060, 007 OF oDlgPvt PIXEL
			@ 025, 220 MSCOMBOBOX oCmbAcao VAR    cCmbAcao ITEMS aIteAcao       SIZE 100, 010 OF oDlgPvt PIXEL
...
Static Function fConfirm(nTipo)
	...
	//Bancos
	If cCmbTip == "01"
		cRotina	:= "MSExecAuto({|x, y| MATA070(x, y)}, aDados, " + cCmbAcao + ") "
		cTabela	:= "SA6"
		cCampoChv	:= ""
	...
	//Clientes
	ElseIf cCmbTip == "02"
		cRotina	:= "MSExecAuto({|x, y| MATA030(x, y)}, aDados, " + cCmbAcao + ") "
		cTabela	:= "SA1"
		cCampoChv	:= "A1_COD"
		cFilialTab	:= FWxFilial(cTabela)
		nModulo	:= 5
	...
	ElseIf cCmbTip == "03"
		cRotina	:= "MSExecAuto({|x, y, z| MATA360(x, y, z)}, aDados, , " + cCmbAcao + ") "
		cTabela	:= "SE4"
		cCampoChv	:= "E4_CODIGO"
		cFilialTab	:= FWxFilial(cTabela)
		nModulo	:= 5
	...
	//Contas a Receber
	ElseIf cCmbTip == "04"
		cRotina	:= "MSExecAuto({|x, y| FINA040(x, y)}, aDados, " + cCmbAcao + ") "
		cTabela	:= "SE1"
		cCampoChv	:= ""
		cFilialTab	:= FWxFilial(cTabela)
		nModulo	:= 6
	...
	//Contas a Pagar
	ElseIf cCmbTip == "05"
		cRotina	:= "MSExecAuto({|x, y, z| FINA050(x, y, z)}, aDados, , " + cCmbAcao + ") "
		cTabela	:= "SE2"
		cCampoChv	:= ""
		cFilialTab	:= FWxFilial(cTabela)
		nModulo	:= 6
	...
	//Fornecedores
	ElseIf cCmbTip == "06"
		cRotina	:= "MSExecAuto({|x, y| MATA020(x, y)}, aDados, " + cCmbAcao + ") "
		cTabela	:= "SA2"
		cCampoChv	:= "A2_COD"
		cFilialTab	:= FWxFilial(cTabela)
		nModulo	:= 2
	...
	//Naturezas
	ElseIf cCmbTip == "07"
		cRotina	:= "MSExecAuto({|x, y| FINA010A(x, y)}, aDados, " + cCmbAcao + ") "
		cTabela	:= "SED"
		cCampoChv	:= "ED_CODIGO"
		cFilialTab	:= FWxFilial(cTabela)
		nModulo	:= 6
	...
	//Produtos
	ElseIf cCmbTip == "08"
		cRotina	:= "MSExecAuto({|x, y| MATA010(x, y)}, aDados, " + cCmbAcao + ") "
		cTabela	:= "SB1"
		cCampoChv	:= "B1_COD"
		cFilialTab	:= FWxFilial(cTabela)
		nModulo	:= 4
	...
	//Saldo Inicial
	ElseIf cCmbTip == "09"
		cRotina	:= "MSExecAuto({|x, y| MATA220(x, y)}, aDados, " + cCmbAcao + ") "
		cTabela	:= "SB9"
		cCampoChv	:= ""
		cFilialTab	:= FWxFilial(cTabela)
		nModulo	:= 4
	...
	ElseIf cCmbTip == "10"
		cRotina	:= "MSExecAuto({|x, y| MATA080(x, y)}, aDados, " + cCmbAcao + ") "
		cTabela	:= "SF4"
		cCampoChv	:= "F4_CODIGO"
		cFilialTab	:= FWxFilial(cTabela)
		nModulo	:= 5
	...
	ElseIf cCmbTip == "11"
		cRotina	:= "MSExecAuto({|x, y| MATA230(x, y)}, aDados, " + cCmbAcao + ") "
		cTabela	:= "SF5"
		cCampoChv	:= "F5_CODIGO"
		cFilialTab	:= FWxFilial(cTabela)
		nModulo	:= 4
	...
	//Transportadoras
	ElseIf cCmbTip == "12"
		cRotina	:= "MSExecAuto({|x, y| MATA050(x, y)}, aDados, " + cCmbAcao + ") "
		cTabela	:= "SA4"
		cCampoChv	:= "A4_COD"
		cFilialTab	:= FWxFilial(cTabela)
		nModulo	:= 5
	...
	//Veículos
	ElseIf cCmbTip == "13"
		cRotina	:= "MSExecAuto({|x, y| OMSA060(x, y)}, aDados, " + cCmbAcao + ") "
		cTabela	:= "DA3"
		cCampoChv	:= "DA3_COD"
		cFilialTab	:= FWxFilial(cTabela)
		nModulo	:= 39
	...
	//Vendedores
	ElseIf cCmbTip == "14"
		cRotina	:= "MSExecAuto({|x, y| MATA040(x, y)}, aDados, " + cCmbAcao + ") "
		cTabela	:= "SA3"
		cCampoChv	:= "A3_COD"
		cFilialTab	:= FWxFilial(cTabela)
		nModulo	:= 5
	...
	ElseIf cCmbTip == "15"
		cRotina	:= "MSExecAuto({|x, y| MATA061(x, y)}, aDados, " + cCmbAcao + ") "
		cTabela	:= "SA5"
		cCampoChv	:= "A5_CODPRF"
		cFilialTab	:= FWxFilial(cTabela)
		nModulo	:= 2
	...
	If nTipo == 1
		...
		If File(cGetArq)
			...
			Else
				...
				If fTelaObs(!Empty(cCampoChv) .and. cCmbAcao != '4')
...
Static Function fTelaObs(lAtivChav)
	...
	aAdd(aHeadNew,{	"Tipo",			"XX_TIPO",		"@!",		1,			0,		"u_zAltTp()",	".T.",	"C", 	"",	"C;N;L;D;M",	"C=Caracter;N=Numérico;L=Lógico;D=Data;M=Memo"})
